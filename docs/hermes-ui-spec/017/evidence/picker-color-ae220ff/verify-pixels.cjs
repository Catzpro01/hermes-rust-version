// Read-only comparison of lower halves of the two retained screenshot panes.
// This includes the complete footer plus blank space; no pixels are rewritten.
const fs = require('fs');
...
(async () => {
  ...
  try {
    ...
    for (const name of ['normal', 'filter', 'no-match']) for (const width of [100, 80]) {
      ...
      const result = await page.evaluate(async data => {
        ...
        const y = Math.floor(canvas.height / 2), height = canvas.height - y;
      ...
      if (!result.equal) throw new Error(`Footer pixel difference: ${id}`);
      cases.push({id, png_sha256: crypto.createHash('sha256').update(raw).digest('hex'), ...result});
    }
  } finally { await browser.close(); }
  const result = {normalization: 'none', comparison: 'unaltered lower halves of matched panes, excluding renderer gutter', cases};
  const output = path.join(__dirname, 'footer-pixels.json');
