//! Spec 017 T04 — rotating tips + composer placeholders (v0.21.0 parity).
//!
//! Verbatim port of Python `hermes_cli/tips.py` (checkout 63279301):
//! [`TIPS`] (380 strings, L11) and [`COMPOSER_PLACEHOLDERS`] (11 strings,
//! L495), sourced from `docs/hermes-ui-spec/017/verbatim/tips.txt` and
//! `composer_placeholders.txt` (this file is generated from them — see the
//! `catalog_matches_verbatim_files` test, which re-parses the catalogs).
//!
//! Parity note (spec §E [KOREKSI]): in v0.21.0 `get_random_tip()` is
//! `random.choice(TIPS)` with no de-duplication, there is **no** `✦ Tip:`
//! display prefix, and neither helper has a caller in `hermes_cli/` — the
//! catalog is *dead code* upstream. The Rust port mirrors that exactly: the
//! data + selectors are available, nothing prints them at startup. Wiring a
//! display surface is a product decision (per /ask-matt), not a parity fix.

// Parity-faithful dead code (see module docs): nothing calls the selectors
// yet, exactly like upstream v0.21.0.
#![allow(dead_code)]

/// `tips.py` `TIPS` — 380 verbatim strings.
pub const TIPS: [&str; 380] = [
...
/// `tips.py` `COMPOSER_PLACEHOLDERS` — 11 verbatim strings.
pub const COMPOSER_PLACEHOLDERS: [&str; 11] = [
...
/// `get_random_tip()` — `random.choice(TIPS)` (no recent-exclusion; the
/// Python `exclude_recent` parameter is reserved and unused).
pub fn random_tip() -> &'static str {
    pick(&TIPS)
}

/// `get_random_composer_placeholder()` — `random.choice(COMPOSER_PLACEHOLDERS)`.
pub fn random_composer_placeholder() -> &'static str {
    pick(&COMPOSER_PLACEHOLDERS)
}

/// Deterministic selector used by both `random_*` helpers and by tests:
/// `catalog[seed % len]`.
pub fn tip_at(seed: u64) -> &'static str {
...
fn pick(catalog: &'static [&'static str]) -> &'static str {
    ...
    // `RandomState` is seeded per process (and mixed per call via the
    // hasher's internal state) — good enough for a cosmetic random choice
    // and avoids pulling a `rand` dependency into the CLI crate.
    ...
    let idx = (h.finish() % catalog.len() as u64) as usize;
    catalog[idx]
}
...
mod tests {
    ...
    /// Minimal parser for the `ast.unparse` list literal in the verbatim
    /// files (single/double-quoted Python string literals, `\'` / `\"` /
    /// `\\` / `\n` escapes, no other escape forms occur in the catalogs).
    fn parse_python_list(src: &str) -> Vec<String> {
        let body: String = src
            .lines()
    ...
    #[test]
    fn catalog_matches_verbatim_files() {
        let tips = parse_python_list(include_str!(
            "../../../../docs/hermes-ui-spec/017/verbatim/tips.txt"
        ));
        assert_eq!(tips.len(), 380, "spec §E: TIPS = 380 strings");
        assert_eq!(tips, TIPS.iter().map(|s| s.to_string()).collect::<Vec<_>>());
        let ph = parse_python_list(include_str!(
    ...
    #[test]
    fn selectors_return_catalog_members() {
        assert!(TIPS.contains(&random_tip()));
        assert!(COMPOSER_PLACEHOLDERS.contains(&random_composer_placeholder()));
        assert_eq!(tip_at(0), TIPS[0]);
        assert_eq!(tip_at(380), TIPS[0]);
        assert_eq!(tip_at(379), TIPS[379]);
    }
}
