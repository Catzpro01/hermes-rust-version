"""One primary regression at the agreed actual CLI/PTY seam.

Requires an already built HERMES_PICKER_BINARY. Capture/setup errors are errors,
not geometry assertion failures. A normal 100x30 pane is the first TDD slice.
"""
import hashlib
import subprocess
import json
...
from check_picker_footer_position import footer_row


class PickerFooterPositionTests(unittest.TestCase):
    def test_footer_is_on_terminal_last_row(self):
        ...
        row = footer_row(case['rust'])
        self.assertEqual(row + 1, 30, 'Python reference: footer is on terminal row30')
